...
def additional_info
	...
	dob_month = gets.strip.to_i
		...
		puts "Please enter a month between 1-12." 
		end
		case dob_month
		when 1
...
def link_accounts_to_user
	...
	@fit_bit = Linked_Accounts.new("Fit Bit", 1)
	@up_by_jawbone = Linked_Accounts.new("UP by Jawbone", 2)

	#how can i create this variable without having to assign a random variable to it before the "until" statement?
	account_to_link =  1000000
	...
	case account_to_link
		when 1
	 	  ...
	 	  puts "Excellent!  You added #{@account.accounts_linked.last.account_name} to your account."
	 	when 2
	 	  @account.accounts_linked.push(@up_by_jawbone)
	 	  puts "Excellent!  You added #{@account.accounts_linked.last.account_name} to your account."
		else
		  puts "I'm sorry, there is an error.  Please close and restart."
	end
end
...
def reddit_display
	...
	puts "Now would you like to see a bunch of random things from Reddit?\n(y)es\n(n)o\n"

	@reddit_confirmation = gets.strip.downcase

	until @reddit_confirmation == 'y' || @reddit_confirmation == 'n'
	puts `clear`
	puts "Oh no!  I don't understand what you wrote. Please confirm you want to see a bunch of reddit health articles.  OK?  \n(y)es?  \n(n)o\n"
	@reddit_confirmation = gets.strip.downcase
	end
